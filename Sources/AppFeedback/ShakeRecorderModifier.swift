...
extension View {
  ...
  public func feedbackRecorder(config: FeedbackConfig?) -> some View {
    #if targetEnvironment(simulator) || targetEnvironment(macCatalyst)
    return self.onAppear {
      print("[AppFeedback] Simulator/Catalyst: feedback recorder disabled (ReplayKit is non-functional).")
    }
    #else
    return Group {
...
/// defaults - `FeedbackConfig` is deliberately not extended per-app for v1.

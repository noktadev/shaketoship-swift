...
public enum FeedbackRecorderError: Error {
  ...
  /// `RPScreenRecorder` reports recording but AppFeedback does not own that
  ...
  case recorderBusy
  /// AppFeedback owned a stranded session but resetting it (`stopCapture`)
...
/// singleton, so `isRecording == true` does NOT prove AppFeedback started it -
...
public actor FeedbackRecorder {
  ...
  public func start(to url: URL) async throws {
    ...
    print("[AppFeedback] ReplayKit is non-functional on the simulator; recording is a no-op.")

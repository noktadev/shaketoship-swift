...
enum FeedbackMediaKind: String, Sendable, Equatable, Hashable, Codable {
  case image
  case video
...
enum FeedbackAttachmentNaming {
  ...
  static let noteFile = "note.txt"

  /// `attachment-{0,1,2}.{jpg,mov}`. The index numbers the PICKED items only,
  /// from zero, in composer order.
  static func attachmentFile(index: Int, kind: FeedbackMediaKind) -> String {
...
struct FeedbackAttachmentCandidate: Sendable, Equatable {
  ...
  init(kind: FeedbackMediaKind, durationSeconds: TimeInterval?, byteCount: Int) {
    self.kind = kind
...
enum FeedbackAttachmentRejection: Equatable, Sendable {
...
enum FeedbackAttachmentBounds {
  ...
  static let maxImageDimension: Double = 1920
